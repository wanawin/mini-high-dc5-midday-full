...
def load_manual_filters_from_file(uploaded_file=None,
    ...
    # Process lines: group logically if multi-line blocks, else each line is separate
    # Simple approach: treat any non-empty normalized line as a separate filter
    for line in raw_lines:
        nt = normalize(line)
        if nt:
            filters.append(nt)
    st.write(f"Loaded {len(filters)} manual filter entries (each line processed)")
    return filters
    ...
    if 'seed sum ≤12' in ft or 'seed sum <=12' in ft or 'seed sum ≤ 12' in ft:
        if seed_sum <= 12:
            ...
            if total < low or total > high:
                return True
    # Add additional parsing here for other filter conditions
    # TODO: extend parsing...
    return False
